...
/// Kimi 限额取数器：coding/v1/usages 解码（usage/detail/totalQuota → 三槽位）、
...
final class KimiLimitsFetcherTests: XCTestCase {
    ...
    func test_usageDecoder_mapsUsageDetailTotalQuotaToSlots() {
        let windows = KimiUsageResponseDecoder.decode([:], usage: usageDict(limit: 4_000_000, used: 1_000_000, reset: "2026-08-22T10:00:00Z"))
        XCTAssertEqual(windows[.session]?.usedPercent, 25)
        XCTAssertEqual(windows[.session]?.limit, 4_000_000)
        XCTAssertEqual(windows[.session]?.used, 1_000_000)
        XCTAssertNil(windows[.session]?.windowSeconds)
        XCTAssertEqual(windows[.session]?.resetAt, ISO8601DateFormatter().date(from: "2026-08-22T10:00:00Z"))
    }

    func test_usageDecoder_detailAndTotalQuotaFillWeeklyAndMonthly() {
        ...
        XCTAssertEqual(windows[.weekly]?.usedPercent, 25, "limits[0].detail → 周窗")
        XCTAssertEqual(windows[.monthly]?.usedPercent, 25, "totalQuota → 月窗")
        XCTAssertEqual(windows[.session]?.usedPercent, 25, "usage → 会话槽（三槽位并存）")
    }

    func test_usageDecoder_unboundedLimitOrUnusableWindow_dropped() {
        XCTAssertNil(KimiUsageResponseDecoder.decodeFromBody([:])[.session], "无 usage → 不产出")
        XCTAssertNil(KimiUsageResponseDecoder.decodeFromBody([
            "usage": ["limit": 0, "used": 100],
        ])[.session], "limit <= 0 丢弃")
        XCTAssertNil(KimiUsageResponseDecoder.decodeFromBody([
            "usage": ["limit": 100, "remaining": "garbage"],
        ])[.session], "used 缺失且 remaining 不可解析 → 丢弃")
    }
    ...
    func test_usageDecoder_derivesUsedFromRemaining() {
        ...
        XCTAssertEqual(windows[.session]?.usedPercent, 75, "used = limit - remaining")
        XCTAssertEqual(windows[.session]?.used, 750)
    }
    ...
    func test_usageDecoder_resetAtMultiFormat() {
        for reset in ["2027-01-15T08:00:00.000Z", 1_800_000_000, 1_800_000_000_000] {
            ...
            XCTAssertEqual(windows[.session]?.resetAt, Date(timeIntervalSince1970: 1_800_000_000), "reset 多格式统一")
        }
    }
    ...
    func test_fetchLimits_freshToken_noRefresh_andBuildsOfficialSnapshot() async throws {
        ...
        XCTAssertEqual(limits?.windows[.weekly]?.usedPercent, 25)

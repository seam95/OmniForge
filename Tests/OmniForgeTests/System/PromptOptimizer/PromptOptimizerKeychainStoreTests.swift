...
/// 提示词优化 API Key 钥匙串封装：真实 SecItem 读写删闭环（每测试唯一 service，teardown 清理）。
/// 无钥匙串权限的宿主（CI/无登录会话）以 `errSecInteractionNotAllowed` 识别并跳过——协议契约由 Fake 覆盖。
final class PromptOptimizerKeychainStoreTests: XCTestCase {
    ...
    private func makeStore() throws -> PromptOptimizerKeychainAPIKeyStore {
        let service = "PromptOptimizerKeychainStoreTests.\(UUID().uuidString)"
        let store = PromptOptimizerKeychainAPIKeyStore(service: service)
        addTeardownBlock {
        ...
        } catch PromptOptimizerKeychainError.keychainUnavailable(let status) where status == errSecInteractionNotAllowed {

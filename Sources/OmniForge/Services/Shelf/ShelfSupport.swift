...
/// One horizontal pointer sample used by the shake-to-open detector.
struct ShelfShakeSample: Equatable {
    let t: TimeInterval
    let x: CGFloat
}

/// Pure shake-to-open algorithm (aligned with vorssaint Shelf):
/// 0.5s window, direction threshold 6pt, reversals ≥ 3, travel > 220, cooldown 1s.
enum ShelfShakeDetector {
    static let window: TimeInterval = 0.5
    static let minSamples = 5
    static let minReversals = 3
    static let minTravel: CGFloat = 220
    static let cooldown: TimeInterval = 1.0
    static let directionThreshold: CGFloat = 6

    /// Returns true when `samples` (plus any recent motion ending at `now`)
    /// represent a summon-worthy shake, and enough time has passed since `lastSummon`.
    static func shouldSummon(samples: [ShelfShakeSample],
                             now: TimeInterval,
                             lastSummon: TimeInterval) -> Bool {
        guard now - lastSummon > cooldown else { return false }

        let windowed = samples.filter { now - $0.t <= window }
        guard windowed.count >= minSamples else { return false }

        var reversals = 0
        var travel: CGFloat = 0
        var lastDirection = 0
        for i in 1..<windowed.count {
            let dx = windowed[i].x - windowed[i - 1].x
            travel += abs(dx)
            let direction = dx > directionThreshold ? 1 : (dx < -directionThreshold ? -1 : 0)
            if direction != 0 {
        ...
        return reversals >= minReversals && travel > minTravel
    }
}

...
///
/// Ported from CapCap `ScrollCropView` (flipped coords, top/bottom crop,
/// min height, edge preview band, full-width `croppedImage()`).
final class ScrollCropView: NSView {
    ...
    /// Test/helper: apply crop fractions in image space (0…1 top→bottom).
    func setCropFractionsForTesting(top: CGFloat, bottom: CGFloat) {
        recomputeLayout(resetCrop: true)
        guard imageFrame.height > 0 else { return }
        let lo = max(0, min(top, bottom))
        let hi = min(1, max(top, bottom))
        cropTop = imageFrame.minY + lo * imageFrame.height
        cropBottom = imageFrame.minY + hi * imageFrame.height
        // Enforce minimum height in view space.
        if cropBottom - cropTop < minimumCropHeight {
            cropBottom = min(imageFrame.maxY, cropTop + minimumCropHeight)
        }
        needsDisplay = true
    }
